...
import Avatar from '../images/6769264_60111.jpg';
...
const Header = () => {
  ...
  if(!user) {
   ...
   {/* Because everytime we refresh the page the user will be automatically logged out and for that */}
  {/* we may save it everytime in the server . But here we will use localstorage to store hte current state of user loggedIn*/}
  ...
  return (
   ...
   { /* desktop and tablet */}
    <div className="hidden md:flex w-full h-full items-center justify-between">
     ...
     <li className='text-base text-textColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer'>{/*<Link to={"/main"*/}Home{/*</Link>*/}</li>
    ...
    { user && user.email ==="nayakswagatika1132@gmail.com" && (
    <Link to={"/createItem"}>
    ...
    <Link to={"/createItem"}>
    <p className='px-4 py-2 flex items-center gap-3 cursor-pointer
    hover:bg-slate-200 transition-all duration-100 ease-in-out text-textColor text-base'>New Item<MdAdd/></p>
    </Link>
    )}
    <ul
     ...
     onClick={()=>setIsMenu(true)}>Home</li>
     <li className='text-base text-textColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer hover:bg-orange-500 px-4 py-2'
    ...
    <p className='m-2 p-2 rounded-md shadow-lg flex items-center gap-3 cursor-pointer
